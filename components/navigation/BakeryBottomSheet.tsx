...
const BakeryBottomSheet: React.FC<BakeryBottomSheetProps> = ({ bakeries, onBakerySelect }) => {
  ...
  const listContainerHeight = bakeries.length > 0 ? (bakeries.length - 1) * CARD_VISIBLE_HEIGHT + CARD_FULL_HEIGHT : 0;
  return (
    <View className="flex-1">
      <ScrollView className="bg-white" contentContainerClassName="pt-4 pb-24">
        <View className="px-4 mb-4 flex-row items-baseline">
          ...
          <Text className="text-sm text-gray-500 ml-2">반경 500m 내 빵집을 추천해드려요</Text>
        </View>
